...
import log4dstm2.AtomicObjectVersion;
import log4dstm2.TestListLogAction;
...
public class Adapter<T> implements dstm2.factory.Adapter<T>, Releasable {
  ...
  AtomicObjectVersion  versionClass;
  ...
  //TestListLogAction la = new TestListLogAction();
  ...
  public Adapter(Class<T> _class) {
    ...
    versionClass = new AtomicObjectVersion();
    iface = _class;
  ...
  /**
   * Tries to open object for writing.
  ...
  Transaction openWrite(Transaction me) {
    ...
    if (!cacheHit) {
      ...
      manager.openSucceeded();
      //logging
      versionClass.getVersionFields(rVersion);
